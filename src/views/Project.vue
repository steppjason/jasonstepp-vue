<template>
	<div class="content" v-if="project">
			...
			<div v-if="project.images" class="project-gallery">
				...
				>
					<img
						...
						:src="getImgUrl(image, project.url, true)"
					/>
				</div>
			</div>
		</transition>
		...
		<div class="full-image" id="fullImage" ref="fullImage">
			<img
				v-if="fullImage"
				:src="getImgUrl(fullImage, project.url, false)"
			/>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		getImgUrl(image, project, thumb) {
			if (thumb) {
				return require("@/assets/img/projects/" +
					project +
					"/" +
					"thumb_" +
					image);
